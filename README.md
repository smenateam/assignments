...
![][arch_schema]
1. Сервис получает информацию о новом заказа, создаёт в БД чеки для всех принтеров для точки указанной в заказе и ставит 
асинхронные задачи на генерацию PDF-файлов для этих чеков. Если у точки нет ни одного принтера - возвращает ошибку.
Если чеки для данного заказа созданы - возвращает ошибку.
...
у которых сгенерирован PDF-файл и статус rendered. После этого запрашивает PDF-версии для каждого чека, этот запрос переводит
...
### Модели
1. Принтер (Printer). Поле api_key принимает уникальные значения, по нему однозначно определяется принтер. Для этой модели должны быть fixtures (по несколько принтеров для нескольких точек).
