...
4. Все инфраструктурные вещи необходимые для сервиса ([PostgreSQL], [Redis], [wkhtmltopdf]) запускать в docker с помощью docker-compose v3, сам проект не нужно оборачивать в docker
5. Помимо API, должна быть админка для обеих моделей, с возможность фильтровать чеки по принтеру, типу и статусу
...
* **Sample Call:**  
  ...
  r = requests.get(SERVICE_URL + '/new_checks', params=params)
  ```
...
* **Sample Call:**  
  ...
  r = requests.get(SERVICE_URL + '/check', params=params)
  ```
