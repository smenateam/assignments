...
Задача - реализовать SPA приложение, в котором пользователь может зарегистрироваться и просмотреть личную информацию о себе

Для простоты разработки данного приложения виде backend-части в данном репозитории представлен файл `app.js`. backend-часть дополнять и изменять `не нужно`. Процесс запуска api на вашей машине описан ниже.
Авторизация и регистрация на backend-части реализована через `jwt-token`.
...
Так же должен быть предусмотрен переход на страницу `/register` в виде кнопки

**Описание страницы `/register`**

Здесь содержится форма регистрации пользователя.

Так же должен быть предусмотрен переход на страницу `/login` в виде кнопки
...
- Убедитесь, что у вас стоит последняя node LTS версии https://nodejs.org/en/
...
2. В терминале/консоли открыть папку [frontend/](frontend/)
...
В результате в коноли получаем output
...
1. Реализацию frontend-части следует делать `в отдельной папке`, а не в этом репозитории. Этот репозиторий содержит в себе только backend-часть в виде api.
...
Если в заголовке не передавать jwt-токен
